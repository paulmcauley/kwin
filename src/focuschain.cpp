...
{
    for (auto it = m_desktopFocusChains.begin(); it != m_desktopFocusChains.end(); ++it) {
...
AbstractClient *FocusChain::getForActivation(uint desktop, int screen) const
{
    ...
    for (int i = chain.size() - 1; i >= 0; --i) {
        ...
        if (tmp->isShown(false) && tmp->isOnCurrentActivity() && (!m_separateScreenFocus || tmp->screen() == screen)) {
            return tmp;
        }
    }
    return nullptr;
...
{
    return c != prev && c->isShown(false) && c->isOnCurrentDesktop() && c->isOnCurrentActivity()
