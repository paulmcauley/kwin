    ...
    if (desktopNameAlignment) {
        for (int screen = 0; screen < effects->numScreens(); screen++) {
            ...
            foreach (EffectFrame *frame, desktopNames) {
                ...
                textArea.adjust(textArea.width() / 10, textArea.height() / 10, -textArea.width() / 10, -textArea.height() / 10);
                int x, y;
...
void DesktopGridEffect::paintWindow(EffectWindow *w, int mask, QRegion region, WindowPaintData &data)
{
    if (timeline.currentValue() != 0 || (isUsingPresentWindows() && isMotionManagerMovingWindows())) {
        if (isUsingPresentWindows() && w == windowMove && wasWindowMove
            && ((!wasWindowCopy && sourceDesktop == paintingDesktop) || (sourceDesktop != highlightedDesktop && highlightedDesktop == paintingDesktop))) {
        ...
        for (int screen = 0; screen < effects->numScreens(); screen++) {
            ...
            if (!quadsAdded) {
                foreach (const WindowQuad &quad, data.quads) {
                    QRect quadRect(w->x() + quad.left(), w->y() + quad.top(), quad.right() - quad.left(), quad.bottom() - quad.top());
            ...
            if (effects->compositingType() == XRenderCompositing) {
                ...
                QPoint screenPos(qRound(screenPosF.x()), qRound(screenPosF.y()));
                QSize screenSize(qRound(interpolate(screenGeom.width(), scaledSize[screen].width(), progress)),
...
void DesktopGridEffect::slotWindowDeleted(EffectWindow *w)
{
    if (w == windowMove)
        windowMove = nullptr;
    if (isUsingPresentWindows()) {
        for (QList<WindowMotionManager>::iterator it = m_managers.begin(), end = m_managers.end(); it != end; ++it) {
            it->unmanage(w);
        }
    }
}
...
void DesktopGridEffect::windowInputMouseEvent(QEvent *e)
{
    ...
    if (e->type() == QEvent::MouseMove) {
        ...
        if (windowMove != nullptr && (me->pos() - dragStartPos).manhattanLength() > QApplication::startDragDistance()) {
        ...
        } else if ((me->buttons() & Qt::LeftButton) && !wasDesktopMove && (me->pos() - dragStartPos).manhattanLength() > QApplication::startDragDistance()) {
            wasDesktopMove = true;
            effects->defineCursor(Qt::ClosedHandCursor);
        }
        if (d != highlightedDesktop) { // Highlight desktop
            if ((me->buttons() & Qt::LeftButton) && isValidMove && !wasWindowMove && d <= effects->numberOfDesktops()) {
                ...
                for (EffectWindowList::const_iterator it = windows.constBegin(), end = windows.constEnd(); it != end; ++it) {
...
QPointF DesktopGridEffect::scalePos(const QPoint &pos, int desktop, int screen) const
{
    ...
    QPointF point(
        interpolate(
            ((screenGeom.width() + unscaledBorder[screen]) * (desktopCell.x() - 1) - (screenGeom.width() + unscaledBorder[screen]) * (activeCell.x() - 1))
                + pos.x(),
            ((scaledSize[screen].width() + border) * (desktopCell.x() - 1) + scaledOffset[screen].x() + (pos.x() - screenGeom.x()) * scale[screen]),
            progress),
        interpolate(
            ((screenGeom.height() + unscaledBorder[screen]) * (desktopCell.y() - 1) - (screenGeom.height() + unscaledBorder[screen]) * (activeCell.y() - 1))
                + pos.y(),
            ((scaledSize[screen].height() + border) * (desktopCell.y() - 1) + scaledOffset[screen].y() + (pos.y() - screenGeom.y()) * scale[screen]),
            progress));
...
QPoint DesktopGridEffect::unscalePos(const QPoint &pos, int *desktop) const
{
    ...
    return QPoint(qBound(screenGeom.x(),
                         qRound(scaledX * (screenGeom.width() + unscaledBorder[screen]) - unscaledBorder[screen] / 2.0 + screenGeom.x()),
                         screenGeom.right()),
                  qBound(screenGeom.y(),
                         qRound(scaledY * (screenGeom.height() + unscaledBorder[screen]) - unscaledBorder[screen] / 2.0 + screenGeom.y()),
                         screenGeom.bottom()));
}
...
EffectWindow *DesktopGridEffect::windowAt(QPoint pos) const
{
    ...
    if (isUsingPresentWindows()) {
        ...
        EffectWindow *w = m_managers.at((desktop - 1) * (effects->numScreens()) + screen).windowAtPoint(pos, false);
        if (w)
            return w;
...
void DesktopGridEffect::setHighlightedDesktop(int d)
{
    ...
    if (highlightedDesktop > 0 && highlightedDesktop <= hoverTimeline.count())
        hoverTimeline[highlightedDesktop - 1]->setCurrentTime(
            qMin(hoverTimeline[highlightedDesktop - 1]->currentTime(), hoverTimeline[highlightedDesktop - 1]->duration()));
...
void DesktopGridEffect::setupGrid()
{
    ...
    for (int i = 0; i < effects->numScreens(); i++) {
        ...
        QSizeF size(double(geom.width()) * sScale, double(geom.height()) * sScale);
        QPointF offset(geom.x() + (geom.width() - size.width() * gridSize.width() - border * (gridSize.width() - 1)) / 2.0,
...
QRectF DesktopGridEffect::moveGeometryToDesktop(int desktop) const
{
    ...
    if (posToDesktop(m_windowMoveGeometry.topLeft() + cursorPos() - m_windowMoveStartPoint) != desktop) {
        ...
        if (posToDesktop(m_windowMoveGeometry.topRight() + cursorPos() - m_windowMoveStartPoint) == desktop) {
            point = unscalePos(m_windowMoveGeometry.topRight() + cursorPos() - m_windowMoveStartPoint) - QPointF(m_windowMoveGeometry.width(), 0) / scaleFactor;
        } else if (posToDesktop(m_windowMoveGeometry.bottomLeft() + cursorPos() - m_windowMoveStartPoint) == desktop) {
            point =
                unscalePos(m_windowMoveGeometry.bottomLeft() + cursorPos() - m_windowMoveStartPoint) - QPointF(0, m_windowMoveGeometry.height()) / scaleFactor;
