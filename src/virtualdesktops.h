...
public:
     ...
     * @see setCurrent
     */
    template<typename Direction> void moveTo(bool wrap = false);
 ...
 */
template<typename Direction> uint getDesktop(int desktop = 0, bool wrap = true);

template<typename Direction> uint getDesktop(int d, bool wrap)
{
    Direction direction;
...
template<typename Direction> void VirtualDesktopManager::moveTo(bool wrap)
{
    Direction functor;
