...
bool BasicEGLSurfaceTextureInternal::create()
{
    if (updateFromFramebuffer()) {
        return true;
    } else if (updateFromRasterbuffer(m_pixmap->image().rect())) {
        return true;
    } else {
...
void BasicEGLSurfaceTextureInternal::update(const QRegion &region)
{
    if (updateFromFramebuffer()) {
        return;
    } else if (updateFromRasterbuffer(region)) {
        return;
    } else {
...
bool BasicEGLSurfaceTextureInternal::updateFromRasterbuffer(const QRegion &region)
{
