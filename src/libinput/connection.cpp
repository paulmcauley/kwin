...
void Connection::doSetup()
{
    ...
    connect(logind, &LogindIntegration::sessionActiveChanged, this, [this](bool active) {
        if (active) {
    ...
    while (!m_eventQueue.isEmpty()) {
        ...
        case LIBINPUT_EVENT_POINTER_AXIS: {
            ...
            for (const InputRedirection::PointerAxis &axis : axes) {
                emit pointerAxisChanged(axis, pe->axisValue(axis), pe->discreteAxisValue(axis), pe->axisSource(), pe->time(), pe->device());
            }
            break;
        }
        ...
        case LIBINPUT_EVENT_TABLET_TOOL_TIP: {
            ...
            const auto *output = static_cast<AbstractWaylandOutput *>(kwinApp()->platform()->enabledOutputs()[client->screen()]);
        ...
        case LIBINPUT_EVENT_TABLET_PAD_BUTTON: {
            ...
            emit tabletPadButtonEvent(tabletEvent->buttonId(), tabletEvent->isButtonPressed(), {event->device()->groupUserData()});
            break;
        }
