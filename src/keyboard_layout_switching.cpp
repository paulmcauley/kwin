    ...
    connect(workspace()->sessionManager(), &SessionManager::prepareSessionSaveRequested, this, [this](const QString &name) {
        ...
        for (auto i = m_layouts.constBegin(); i != m_layouts.constEnd(); ++i) {
            if (const uint layout = *i) {
                m_config.writeEntry(defaultLayoutEntryKey() % QLatin1String(QByteArray::number(i.key()->x11DesktopNumber())), layout);
            }
        }
    });
    ...
    connect(workspace()->sessionManager(), &SessionManager::loadSessionRequested, this, [this, xkb](const QString &name) {
        ...
        if (xkb->numberOfLayouts() > 1) {
            ...
            for (KWin::VirtualDesktop *const desktop : desktops) {
                const uint layout = m_config.readEntry(defaultLayoutEntryKey() % QLatin1String(QByteArray::number(desktop->x11DesktopNumber())), 0u);
                if (layout) {
...
namespace
{
template<typename T, typename U> quint32 getLayout(const T &layouts, const U &reference)
{
        ...
        for (auto i = m_layouts.constBegin(); i != m_layouts.constEnd(); ++i) {
            if (const uint layout = *i) {
                ...
                if (!desktopFileName.isEmpty()) {
                    m_config.writeEntry(defaultLayoutEntryKey() % QLatin1String(desktopFileName), layout);
                }
            }
        }
    });
    ...
    connect(workspace()->sessionManager(), &SessionManager::loadSessionRequested, this, [this, xkb](const QString &name) {
        ...
        if (xkb->numberOfLayouts() > 1) {
            ...
            for (const QString &key : keyList) {
                m_layoutsRestored.insert(key.midRef(keyPrefix.size()).toLatin1(), m_config.readEntry(key, 0));
            }
        }
