...
{
    const char *const policies[] = {"NoPlacement",
                                    "Default",
...
void Workspace::slotWindowPackLeft()
{
    if (active_client && active_client->isMovable())
        active_client->packTo(packPositionLeft(active_client, active_client->frameGeometry().left(), true), active_client->y());
}

void Workspace::slotWindowPackRight()
{
    if (active_client && active_client->isMovable())
        active_client->packTo(packPositionRight(active_client, active_client->frameGeometry().right(), true) - active_client->width() + 1, active_client->y());
}

void Workspace::slotWindowPackUp()
{
    if (active_client && active_client->isMovable())
        active_client->packTo(active_client->x(), packPositionUp(active_client, active_client->frameGeometry().top(), true));
}

void Workspace::slotWindowPackDown()
{
    if (active_client && active_client->isMovable())
        active_client->packTo(active_client->x(), packPositionDown(active_client, active_client->frameGeometry().bottom(), true) - active_client->height() + 1);
}
...
void AbstractClient::growVertical()
{
    ...
    if (frameGeometry().size() == adjsize && geom.size() != adjsize && resizeIncrements().height() > 1) { // take care of size increments
        ...
        // check that it hasn't grown outside of the area, due to size increments
        if (workspace()->clientArea(MovementArea, QPoint(frameGeometry().center().x(), (y() + newbottom) / 2), desktop()).bottom() >= newbottom)
...
#define FLAG(name) QuickTileMode(QuickTileFlag::name)
    if (!m_quickTileCombineTimer->isActive()) {
        ...
        if (((m_lastTilingMode == FLAG(Left) || m_lastTilingMode == FLAG(Right)) && (mode == FLAG(Top) || mode == FLAG(Bottom)))
...
int Workspace::packPositionLeft(const AbstractClient *client, int oldX, bool leftEdge) const
{
    ...
    if (oldX <= newX) { // try another Xinerama screen
        newX = clientArea(MaximizeArea, QPoint(client->frameGeometry().left() - 1, client->frameGeometry().center().y()), client->desktop()).left();
    }
...
int Workspace::packPositionRight(const AbstractClient *client, int oldX, bool rightEdge) const
{
    ...
    if (oldX >= newX) { // try another Xinerama screen
        newX = clientArea(MaximizeArea, QPoint(client->frameGeometry().right() + 1, client->frameGeometry().center().y()), client->desktop()).right();
    }
    ...
    for (auto it = m_allClients.constBegin(), end = m_allClients.constEnd(); it != end; ++it) {
        ...
        if (x < newX && x > oldX
            && !(client->frameGeometry().top() > (*it)->frameGeometry().bottom() || client->frameGeometry().bottom() < (*it)->frameGeometry().top())) {
            newX = x;
        }
    }
    return newX;
...
int Workspace::packPositionUp(const AbstractClient *client, int oldY, bool topEdge) const
{
    ...
    if (oldY <= newY) { // try another Xinerama screen
        newY = clientArea(MaximizeArea, QPoint(client->frameGeometry().center().x(), client->frameGeometry().top() - 1), client->desktop()).top();
    }
...
int Workspace::packPositionDown(const AbstractClient *client, int oldY, bool bottomEdge) const
{
    ...
    if (oldY >= newY) { // try another Xinerama screen
        newY = clientArea(MaximizeArea, QPoint(client->frameGeometry().center().x(), client->frameGeometry().bottom() + 1), client->desktop()).bottom();
    }
    ...
    for (auto it = m_allClients.constBegin(), end = m_allClients.constEnd(); it != end; ++it) {
        ...
        if (y < newY && y > oldY
            && !(client->frameGeometry().left() > (*it)->frameGeometry().right() || client->frameGeometry().right() < (*it)->frameGeometry().left())) {
            newY = y;
        }
    }
    return newY;
