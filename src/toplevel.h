...
public:
     ...
     */
    template<class T, class U> static T *findInList(const QList<T *> &list, std::function<bool(const U *)> func);
...
{
    return kwinApp()->operationMode() == Application::OperationModeWaylandOnly || kwinApp()->operationMode() == Application::OperationModeXwayland
        // Wayland
        ? desktops().isEmpty()
        // X11
...
template<class T, class U> inline T *Toplevel::findInList(const QList<T *> &list, std::function<bool(const U *)> func)
{
