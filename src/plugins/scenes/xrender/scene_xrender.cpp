...
PlatformSurfaceTexture *SceneXrender::createPlatformSurfaceTextureX11(SurfaceTextureX11 *texture)
{
    return new PlatformXrenderSurfaceTextureX11(texture);
}
...
#undef RENDER_SHADOW_TILE
        ...
        if (!(client && client->isShade())) {
            ...
            if (data.crossFadeProgress() < 1.0 && data.crossFadeProgress() > 0.0) {
                SurfaceTexture *previous = surfaceItem()->previousTexture();
