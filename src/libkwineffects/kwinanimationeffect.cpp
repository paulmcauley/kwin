...
#define RELATIVE_XY(_FIELD_)                                                                                                                                   \
    const bool relative[2] = {static_cast<bool>(metaData(Relative##_FIELD_##X, meta)), static_cast<bool>(metaData(Relative##_FIELD_##Y, meta))}
...
void AnimationEffect::validate(Attribute a, uint &meta, FPx2 *from, FPx2 *to, const EffectWindow *w) const
{
    if (a < NonFloatBase) {
        if (a == Scale) {
            ...
            if (to && to->isValid()) {
                RELATIVE_XY(Target);
                to->set(relative[0] ? (*to)[0] * area.width() / w->width() : (*to)[0], relative[1] ? (*to)[1] * area.height() / w->height() : (*to)[1]);
            }
        } else if (a == Rotation) {
            if (from && !from->isValid()) {
        ...
        if (from) {
            if (from->isValid()) {
                RELATIVE_XY(Source);
                from->set(relative[0] ? area.x() + (*from)[0] * area.width() : (*from)[0], relative[1] ? area.y() + (*from)[1] * area.height() : (*from)[1]);
            } else {
                from->set(pt.x(), pt.y());
        ...
        if (to) {
            if (to->isValid()) {
                RELATIVE_XY(Target);
                to->set(relative[0] ? area.x() + (*to)[0] * area.width() : (*to)[0], relative[1] ? area.y() + (*to)[1] * area.height() : (*to)[1]);
            } else {
                to->set(pt.x(), pt.y());
        ...
        if (from) {
            if (from->isValid()) {
                RELATIVE_XY(Source);
                from->set(relative[0] ? (*from)[0] * area.width() : (*from)[0], relative[1] ? (*from)[1] * area.height() : (*from)[1]);
            } else {
        ...
        if (to) {
            if (to->isValid()) {
                RELATIVE_XY(Target);
                to->set(relative[0] ? (*to)[0] * area.width() : (*to)[0], relative[1] ? (*to)[1] * area.height() : (*to)[1]);
            } else {
                                   ...
                                   bool keepAlive)
{
    ...
    if (d->m_animations.isEmpty()) {
        connect(effects, &EffectsHandler::windowGeometryShapeChanged, this, &AnimationEffect::_expandedGeometryChanged);
        connect(effects, &EffectsHandler::windowStepUserMovedResized, this, &AnimationEffect::_expandedGeometryChanged);
        connect(effects, &EffectsHandler::windowPaddingChanged, this, &AnimationEffect::_expandedGeometryChanged);
    }
...
bool AnimationEffect::redirect(quint64 animationId, Direction direction, TerminationFlags terminationFlags)
{
    ...
    for (auto entryIt = d->m_animations.begin(); entryIt != d->m_animations.end(); ++entryIt) {
        auto animIt = std::find_if(entryIt->first.begin(), entryIt->first.end(), [animationId](AniData &anim) {
...
QRect AnimationEffect::clipRect(const QRect &geo, const AniData &anim) const
{
    ...
    const QRect center = geo.adjusted(clip.width() / 2, clip.height() / 2, -(clip.width() + 1) / 2, -(clip.height() + 1) / 2);
    const int x[2] = {xCoord(center, metaData(SourceAnchor, anim.meta)), xCoord(center, metaData(TargetAnchor, anim.meta))};
    const int y[2] = {yCoord(center, metaData(SourceAnchor, anim.meta)), yCoord(center, metaData(TargetAnchor, anim.meta))};
...
{
    disconnect(effects, &EffectsHandler::windowGeometryShapeChanged, this, &AnimationEffect::_expandedGeometryChanged);
    disconnect(effects, &EffectsHandler::windowStepUserMovedResized, this, &AnimationEffect::_expandedGeometryChanged);
    disconnect(effects, &EffectsHandler::windowPaddingChanged, this, &AnimationEffect::_expandedGeometryChanged);
}
            ...
            for (QList<AniData>::const_iterator anim = entry->first.constBegin(); anim != entry->first.constEnd(); ++anim) {
                ...
                case Position: {
                    ...
                    if (anim->from[0] >= 0.0 && anim->to[0] >= 0.0) {
                        ...
                        const int x[2] = {xCoord(geo, metaData(SourceAnchor, anim->meta)), xCoord(geo, metaData(TargetAnchor, anim->meta))};
                        data.translate(dest - (x[0] + prgrs * (x[1] - x[0])));
                    }
                    if (anim->from[1] >= 0.0 && anim->to[1] >= 0.0) {
                        float dest = interpolated(*anim, 1);
                        const int y[2] = {yCoord(geo, metaData(SourceAnchor, anim->meta)), yCoord(geo, metaData(TargetAnchor, anim->meta))};
                ...
                case Rotation: {
                    ...
                    const uint sAnchor = metaData(SourceAnchor, anim->meta), tAnchor = metaData(TargetAnchor, anim->meta);
